...
void make_vis(recycle_memory<int>& r3) {
    ...
    for (int i = 0; i < 5; i++) {
        reuseable_buffer<int>* r = r3.new_ptr();
        r->ptr = &i;
        r3.queue_ptr(r);
    }
}
...
void read_vis(recycle_memory<int>& r3, reuseable_buffer<int>* vis) {
    //std::ofstream file;
    //file.open("test.txt");
...
void change_vis(recycle_memory<int>& r3) {
    while (true) { // what is the condition that needs to be here? will this need to be a condition?
        reuseable_buffer<int> *vis = r3.start_operation();
        int i = *vis->ptr;
        *vis->ptr = i+1;
        // read_vis should actually be the type of vis's pointer
        recycle_memory<int>::func f = read_vis;
        std::vector<recycle_memory<int>::func> v {f};
